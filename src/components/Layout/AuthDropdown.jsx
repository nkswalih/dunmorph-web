...
const AuthDropdown = ({ isOpen, onClose }) => {
  return (
    <div 
      className={`absolute right-0 top-full mt-6 w-[380px] bg-cream/95 backdrop-blur-md border border-gray-200 z-50 p-8 shadow-xl transition-all duration-300 ease-out origin-top ${isOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-4'}`}
    ...
    >
      <div className="flex items-center gap-2 mb-6 text-gray-900">
        ...
        <span className="font-serif text-2xl tracking-wide">Sign In</span>
        <Link to="/register" className="ml-auto text-xs uppercase tracking-widest text-gray-500 hover:text-gold border-b border-transparent hover:border-gold transition-all">
          Create Account
        </Link>
      </div>
      ...
      <form className="space-y-4">
        ...
        <div className="flex items-center justify-between mt-2">
          <label className="flex items-start gap-2 cursor-pointer group">
            ...
            <span className="text-xs text-gray-500 max-w-[200px] leading-relaxed group-hover:text-gray-900">
              Remember me - I want Dunmorph to personalise my shopping experience
            </span>
          </label>
        </div>
        ...
        <button className="w-full bg-[#0a1429] text-white py-4 text-xs tracking-widest uppercase hover:bg-gold transition-colors mt-6 font-semibold">
          Sign In
        </button>
      </form>
      ...
      <div className="relative mt-8 mb-6">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-4 bg-cream text-gray-400 text-xs italic backdrop-blur-md">or</span>
        </div>
      </div>

      <div className="flex justify-between items-center gap-4">
        {/* Facebook */}
        <button className="flex flex-col items-center justify-center gap-2 group w-1/3">
